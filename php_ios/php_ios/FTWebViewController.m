...
@interface FTWebViewController ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,UINavigationControllerDelegate,UINavigationBarDelegate>
...
-(UIBarButtonItem*)customBackBarItem{
    if (!_customBackBarItem) {
        ...
        [leftBtn setFrame:CGRectMake(0, 2, 50, 30)];
        [leftBtn setImage:[UIImage imageNamed:@"web_back"] forState:UIControlStateNormal];
        leftBtn.imageEdgeInsets = UIEdgeInsetsMake(0, -5, 0, 0);
        
        [leftBtn addTarget:self action:@selector(customBackItemClicked) forControlEvents:UIControlEventTouchUpInside];
        _customBackBarItem = [[UIBarButtonItem alloc] initWithCustomView:leftBtn];
    }
...
- (UIProgressView *)progressView{
    if (!_progressView) {
        ...
        //_progressView.progressTintColor = UIColorFromRGB(system_color);
    }
...
-(UIBarButtonItem*)closeButtonItem{
    if (!_closeButtonItem) {
        _closeButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"nav_close", nil) style:UIBarButtonItemStylePlain target:self action:@selector(closeItemClicked)];
    }
