...
- (NSString *)downloadDirectory
{
    if (!_downloadDirectory) {
        _downloadDirectory = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"NSDocumentDirectory"];
...
// 下载model字典，key为downloadUrl，value为DownloadModel
...
- (TYDownloadModel *)startDownloadURLString:(NSString *)URLString toDestinationPath:(NSString *)destinationPath progress:(TYDownloadProgressBlock)progress state:(TYDownloadStateBlock)state
{
    ...
    // 如果downloadModel为空，或者downloadModel的filePath与要下载的路径不一致，则重新创建downloadModel
...
- (void)resumeWithDownloadModel:(TYDownloadModel *)downloadModel{
    ...
    if (!downloadModel.task || downloadModel.task.state == NSURLSessionTaskStateCanceling) {
        ...
        // 设置请求头
        //文件下载，一般的下载可以直接下载。而针对视频或者下载软件等下载需要分段下载。以下就是模拟的分段下载，即利用request设置Range，这个Range就是一个“bytes=1-1024”之类规定请求或者说下载的范围。
    ...
    if ([self.fileManager fileExistsAtPath:downloadModel.filePath]) {
        ...
        if ([self.fileManager fileExistsAtPath:[self fileSizePathWithDownloadModel:downloadModel]]) {
            @synchronized (self) {
                ...
                //NSLog(@"cft-dict:%@",dict);
                // "http://dlsw.baidu.com/sw-search-sp/soft/b4/25734/itunes12.3.1442478948.dmg" = 250744828;
                [dict removeObjectForKey:downloadModel.downloadURL];
                //NSLog(@"cft-dict after:%@",dict);
                // 将更新后的dict写入plist文件中
...
{
    // http://flv3.bn.netease.com/tvmrepo/2018/3/2/6/EDCIBCT26/SD/EDCIBCT26-mobile.mp4
    NSLog(@"cft-taskDescription:%@",dataTask.taskDescription);
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^(){// 更新下载进度
